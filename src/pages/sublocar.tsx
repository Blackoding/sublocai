...
const ConsultorioCardWithRating = ({
  consultorio,
}: {
  ...
  return (
    <SublocationCard
      ...
      imageUrl={
        consultorio.images && consultorio.images.length > 0
          ? consultorio.images[0]
          : "/office-empty.jpg"
      }
...
const SublocarPage = () => {
  ...
  const filteredConsultorios = useMemo(() => {
    return consultorios.filter((consultorio) => {
      ...
      const matchesRegion =
        selectedRegion === "" || consultorio.city === selectedRegion;
      ...
      const matchesFeatures =
        ...
        selectedFeatures.every((feature) =>
          consultorio.features.includes(feature),
        );
          ...
          <div className="lg:w-80 flex-shrink-0">
            <div className="bg-white rounded-3xl shadow-md p-6 sticky top-24">
              ...
              <div className="mb-6">
                ...
                <div
                  className={`space-y-2 ${showAllFeatures ? "max-h-48 overflow-y-auto pr-2" : ""}`}
                >
                  {(showAllFeatures
                    ? ALL_FEATURES
                    : ALL_FEATURES.slice(0, 3)
                  ).map((feature) => (
                    <Checkbox
                      key={feature.value}
                      label={feature.label}
          ...
          <div className="flex-1">
            ...
            <div className="mb-6">
              <p className="text-gray-600">
                {filteredConsultorios.length} espaço
                {filteredConsultorios.length !== 1 ? "s" : ""} encontrado
                {filteredConsultorios.length !== 1 ? "s" : ""}
              </p>
            </div>
            ...
            {isLoading ? (
              <Loading
                ...
                description="Buscando espaços disponíveis para sublocação."
              />
