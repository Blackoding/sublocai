...
export default async function handler(
  ...
  try {
    ...
    const isParticipant =
      appointment.user_id === userId || clinic.user_id === userId;
    if (!isParticipant) {
      res
        .status(403)
    ...
    const senderIds = Array.from(
      new Set(
        ((messagesRows || []) as MessageRow[]).map(
    ...
    if (senderIds.length > 0) {
      ...
      usersById = ((usersRows || []) as UserRow[]).reduce<
    ...
    const data: ChatMessage[] = ((messagesRows || []) as MessageRow[]).map(
      (message) => {
