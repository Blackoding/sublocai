...
export default async function handler(
  ...
  try {
    ...
    if (!isNonEmptyString(senderId) || !isNonEmptyString(appointmentId) || !isNonEmptyString(content)) {
    ...
    if (containsContactData(sanitizedContent)) {
      res.status(400).json({ error: 'Não é permitido compartilhar telefone ou e-mail no chat' });
      return;
    }
    ...
    if (senderId === bookerId) {
      ...
      res.status(403).json({ error: 'Sem permissão para enviar mensagem neste chat' });
      return;
    }
    ...
    const senderName = senderData
      ? senderData.trade_name || senderData.company_name || senderData.full_name || 'Usuário'
      : 'Usuário';
    ...
    if (insertError || !insertedData) {
      res.status(400).json({ error: insertError?.message || 'Erro ao enviar mensagem' });
      return;
    }
