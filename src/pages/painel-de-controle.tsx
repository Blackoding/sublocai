...
const ControlPanelPage = () => {
    ...
    const loadClinics = async () => {
      ...
      try {
        ...
        if (result.success && result.clinics) {
          const formattedClinics: DashboardClinic[] = result.clinics.map(
            (clinic) => ({
              id: clinic.id || "",
              title: clinic.title,
  ...
  useEffect(() => {
    const loadAppointments = async () => {
      ...
      try {
        ...
        const received = completedAppointments.reduce(
          (sum, appointment) => sum + (appointment.value || 0),
          0,
        );
        const pending = pendingAppointments.reduce(
          (sum, appointment) => sum + (appointment.value || 0),
          0,
        );
  ...
  const isCompanyFree =
    isCompany && (user?.planEmpresa === "free" || !user?.planEmpresa);
  const isCompanyBasic =
    isCompany && (user?.planEmpresa === "basic" || user?.planEmpresa === "pro");
  ...
  const totalInvestment = completedAppointments.reduce(
    (sum, appointment) => sum + (appointment.value || 0),
    0,
  );
  ...
  const recentWeekData = Array.from({ length: 7 }).map((_, index) => {
    ...
    const total = appointments.filter(
      (appointment) =>
        appointment.date && appointment.date.slice(0, 10) === currentDateKey,
    ).length;
  ...
  const maxWeekAppointments = Math.max(
    ...recentWeekData.map((item) => item.total),
    1,
  );
  ...
  const maxMonthlyAppointments = Math.max(
    ...monthlyTrendData.map((item) => item.total),
    1,
  );
  ...
  const weekdayPerformanceMap = completedAppointments.reduce<
  ...
  const bottomRevenueClinic =
    clinicRevenueRanking.length > 0
      ? clinicRevenueRanking[clinicRevenueRanking.length - 1]
      : null;
  ...
  const conversionRate =
    stats.totalViews > 0 ? (stats.totalBookings / stats.totalViews) * 100 : 0;
  const occupancyRate =
  ...
  const confirmDeleteClinic = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showToast(
          `Erro ao excluir espaço: ${result.error || "Tente novamente."}`,
          "error",
        );
      }
    } catch {
      showToast("Erro ao excluir espaço. Tente novamente.", "error");
    }
  ...
  const handleToggleStatus = async (id: string) => {
    try {
      ...
      const newStatus: DashboardClinic["status"] =
        clinic.status === "ativo" ? "inativo" : "ativo";
      const targetStatus = newStatus === "ativo" ? "active" : "inactive";
      const { clinicUtils } = await import("@/services/clinicService");
      const result = await clinicUtils.updateClinic(id, {
        status: targetStatus,
      });
      ...
      if (result.success) {
        ...
        showToast(
          newStatus === "ativo"
            ? "Espaço ativado com sucesso!"
            : "Espaço desativado com sucesso!",
          "success",
        );
      } else {
        showToast(
          `Erro ao alterar status: ${result.error || "Tente novamente."}`,
          "error",
        );
      }
    } catch {
  ...
  const headerDescription = isCompanyFreePanel
    ? "Empresário Básico: visão completa com gráficos, métricas e indicadores"
    : isCompanyBasicPanel
      ? "Empresário Básico: visão completa com gráficos, métricas e indicadores"
      : "Profissional: acompanhe seus agendamentos e encontre espaços para sublocar";
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {isCompanyBasicPanel && (
          <>
            <div className="relative mb-8 overflow-hidden rounded-2xl bg-gradient-to-r from-green-500 to-blue-600 p-6 text-white shadow-lg md:p-8">
              <Link
                href="/painel-de-controle/financeiro"
                className="absolute right-4 top-4 rounded-full border border-white/80 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-white/15 md:right-6 md:top-6"
              >
                Ver mais
              </Link>
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3 md:pr-28">
                <div>
                  <p className="text-sm text-green-100 mb-1">Valor recebido</p>
                  <p className="text-3xl font-bold">
                    {financialData.isLoading
                      ? "Carregando..."
                      : formatCurrency(financialData.received)}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-3xl font-bold">
                    {financialData.isLoading
                      ? "Carregando..."
                      : formatCurrency(financialData.pending)}
                  </p>
                </div>
                <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <div className="bg-white rounded-2xl shadow-sm p-6">
                <p className="text-sm font-medium text-gray-600">Espaços</p>
            ...
            {clinicRevenueRanking.length > 1 && (
              <div className="bg-white rounded-2xl shadow-sm p-6 mb-8">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">
                  Ranking de faturamento por espaço
                </h2>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  <div className="rounded-xl border border-green-200 bg-green-50 p-4">
                    <p className="text-xs font-semibold text-green-700 mb-1">
                      Espaço que mais fatura
                    </p>
                    <p className="text-base font-semibold text-gray-900">
                      {topRevenueClinic?.clinicTitle || "-"}
                    </p>
                  ...
                  <div className="rounded-xl border border-orange-200 bg-orange-50 p-4">
                    <p className="text-xs font-semibold text-orange-700 mb-1">
                      Espaço que menos fatura
                    </p>
                    <p className="text-base font-semibold text-gray-900">
                      {bottomRevenueClinic?.clinicTitle || "-"}
                    </p>
                ...
                <div className="space-y-4">
                  {clinicRevenueRanking.slice(0, 5).map((item) => {
                    const percent =
                      maxRevenueValue > 0
                        ? (item.revenue / maxRevenueValue) * 100
                        : 0;
            ...
            {isLoadingAppointments ? (
              <div className="bg-white rounded-2xl shadow-sm p-6 mb-8">
                <Loading
                  ...
                  description="Aguarde enquanto buscamos seus agendamentos."
                />
              </div>
            ) : appointmentStats.total === 0 ? (
              <div className="bg-white rounded-2xl shadow-sm p-6 mb-8">
                <div className="rounded-xl bg-gray-50 border border-gray-100 p-8 text-center flex flex-col items-center justify-center gap-4">
                  <p className="text-gray-600">
                    Você ainda não possui agendamentos. Busque um espaço para
                    começar.
                  </p>
                  <Link href="/sublocar">
              ...
              <>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm p-6">
                    ...
                    <div className="space-y-3">
                      {recentWeekData.map((item) => {
                        const percent =
                          (item.total / maxWeekAppointments) * 100;
                        return (
                          <div key={item.dayLabel}>
                            ...
                            <div className="w-full h-2 rounded-full bg-gray-100">
                              <div
                                className="h-2 rounded-full bg-blue-500"
                                style={{
                                  width: `${Math.max(percent, item.total > 0 ? 8 : 0)}%`,
                                }}
                              />
                            </div>
                          </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm p-6">
                    ...
                    <div className="space-y-3">
                      {statusDistributionData.map((item) => {
                        const percent =
                          appointmentStats.total > 0
                            ? (item.value / appointmentStats.total) * 100
                            : 0;
                        return (
                          <div key={item.label}>
                            <div className="flex items-center justify-between text-sm mb-1">
                            ...
                            <div className="w-full h-2 rounded-full bg-gray-100">
                              <div
                                ...
                                style={{
                                  width: `${Math.max(percent, item.value > 0 ? 8 : 0)}%`,
                                }}
                              />
                            </div>
                          </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm p-6 mb-8">
                  ...
                  <div className="grid grid-cols-6 gap-3 items-end h-44">
                    {monthlyTrendData.map((item) => {
                      const heightPercent =
                        (item.total / maxMonthlyAppointments) * 100;
                      return (
                        <div
                          key={item.label}
                        ...
                        >
                          <div
                            ...
                            style={{
                              height: `${Math.max(heightPercent, item.total > 0 ? 10 : 0)}%`,
                            }}
                          />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                  ...
                  <div className="bg-white rounded-2xl shadow-sm p-6">
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      {uniqueClinicsVisited}
                    </p>
                  </div>
                  <div className="bg-white rounded-2xl shadow-sm p-6">
                    <p className="text-sm font-medium text-gray-600">
                      Melhor dia de atendimento
                    </p>
                ...
                <div className="bg-white rounded-2xl shadow-sm p-6 mb-8">
                  ...
                  <p className="text-gray-600">
                    Você já concluiu <strong>{attendedPatients}</strong>{" "}
                    atendimentos e acumulou aproximadamente{" "}
                    <strong>{workedHours}h</strong> de trabalho no período
                    analisado.
                  </p>
                  <p className="text-gray-600 mt-2">
                    Investimento estimado em atendimentos concluídos:{" "}
        ...
        {!isProfessionalPanel && (
          <div className="space-y-6">
            {isLoadingClinics ? (
              <Loading
                message="Carregando espaços..."
                description="Aguarde enquanto buscamos seus espaços."
              />
              ...
              <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
                ...
                <p className="text-gray-600 mb-6">
                  Você ainda não cadastrou espaço.
                </p>
        ...
        {isCompanyFreePanel && (
          <div className="bg-white rounded-2xl shadow-sm p-6 mt-8">
            ...
            {isLoadingAppointments ? (
              <Loading
                message="Carregando agendamentos..."
                description="Aguarde enquanto buscamos os agendamentos."
              />
                ...
                {appointments.slice(0, 6).map((appointment) => (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      <div>
                        <p className="font-semibold text-gray-900">
                          {appointment.clinic_title || "Espaço"}
                        </p>
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-xs font-semibold px-2.5 py-1 rounded-full ${getAppointmentStatusStyle(appointment.status)}`}
                        >
      ...
      >
        <div className="text-center py-4">
          ...
          <p className="text-sm text-gray-500 mb-4">
            <strong>&quot;{deleteModal.clinicTitle}&quot;</strong> será
            permanentemente removido.
          </p>
          <p className="text-xs text-gray-400">
            Esta ação não pode ser desfeita e todos os dados relacionados serão
            perdidos.
          </p>
        </div>
      </Modal>
